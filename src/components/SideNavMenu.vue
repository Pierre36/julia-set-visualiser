...
<template>
  <nav ref="nav">
    <ul>
      <li v-for="panel in displayedPanels" class="navItem" :class="{ 'selected': currentPanel == panel.id && !sidePanelCollapsed }">
      ...
      <li ref="moreListItem" id="moreWrapper" class="navItem" :class="{ 'hidden': hiddenPanels.length == 0 }">
        ...
        <ul id="moreMenu" ref="moreMenu" class="dropdownMenu" v-if="!moreMenuCollapsed">
          <li v-for="panel in hiddenPanels" :class="{ 'selected': currentPanel == panel.id && !sidePanelCollapsed }" @click="changePanel(panel.id)">
            <span class="dot"></span>
